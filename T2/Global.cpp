...
bool IsOldFunctionExt(string filename, string id, string address, bool condition)
{
	...
	if (((address.find("(e", 0) != string::npos) || (address.find("(m", 0) != string::npos) || (address.find("(r", 0) != string::npos) || (address.find("(y", 0) != string::npos)) && (count(tempadd.begin(), tempadd.end(), '>') == 2) && (filename != "ZeroRegion"))
	{
		return IsOldRegion(id, address, false);
	}
	else if ((address.find("(cj", 0) != string::npos) || (address.find("(i", 0) != string::npos) && (count(tempadd.begin(), tempadd.end(), '>') == 3) && (filename != "ZeroRegion"))
	{
...
string CrossReferencing(string id, string address, int functionlayer, bool compare, bool special) // whether it is foreign principle or usual cross referencing
{
	if (compare) // foreign principle 
	{
		...
		else if (special && !IsForeign[parent[id]])
		{
			pass = true;
		}
	...
	else
	{
		if (address.find("region", 0) == string::npos)
		{
			...
			for (auto& it = addressID.begin(); it != addressID.end(); ++it) // update new address to existing function
			{
				if (it->second == id) oldAdd.push_back(it->first);
			}

			unordered_map<string, string> tempmap;

			for (auto& it = addressID.begin(); it != addressID.end(); ++it) // update new address to children of the existing function
			{
				for (auto& o_add : oldAdd)
				{
					string tempadd = it->first;

					if (tempadd.find(o_add, 0) != string::npos)
					{
						tempadd.replace(0, o_add.length(), address);
						tempmap[tempadd] = it->second;
						break;
					}
				}
		...
		if (addressID[address].empty()) addressID[address] = id;
	}
