...
hkbrigidbodyragdollcontrolsmodifier::hkbrigidbodyragdollcontrolsmodifier(string filepath, string id, string preaddress, int functionlayer, bool compare)
{
	...
	address = preaddress + "bs" + to_string(functionlayer) + ">";
...
void hkbrigidbodyragdollcontrolsmodifier::nonCompare(string filepath, string id)
{
	...
	if (!FunctionLineOriginal[id].empty())
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
				bones = line.substr(25, line.find("</hkparam>") - 25);
				if (bones != "null")
				{
					referencingIDs[bones].push_back(id);
				}
			}
	...
	{
		cout << "ERROR: hkbRigidBodyRagdollControlsModifier Outputfile(File: " << filepath << ", ID: " << id << ")" << endl;
		Error = true;
	}
...
void hkbrigidbodyragdollcontrolsmodifier::Compare(string filepath, string id)
{
	...
	if (!FunctionLineEdited[id].empty())
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			else if (line.find("<hkparam name=\"bones\">", 0) != string::npos)
			{
				...
				if (bones != "null")
				{
					if (!exchangeID[bones].empty())
					{
						int tempint = line.find(bones);
						bones = exchangeID[bones];
						line.replace(tempint, line.find("</hkparam>") - tempint, bones);
					}
					parent[bones] = id;
					referencingIDs[bones].push_back(id);
				}
	...
	if (IsOldFunction(filepath, id, address)) // is this new function or old
	{
		...
		if (bones != "null")
		{
			referencingIDs[bones].pop_back();
			referencingIDs[bones].push_back(tempid);
		}
		...
		{
			cout << "ERROR: hkbRigidBodyRagdollControlsModifier Inputfile(File: " << filepath << ", newID: " << id << ", oldID: " << tempid << ")" << endl;
			Error = true;
		}
		...
		{
			cout << "ERROR: hkbRigidBodyRagdollControlsModifier Outputfile(File: " << filepath << ", newID: " << id << ", oldID: " << tempid << ")" << endl;
			Error = true;
		}
...
void hkbrigidbodyragdollcontrolsmodifier::Dummy(string id)
{
	...
	{
		cout << "ERROR: Dummy hkbRigidBodyRagdollControlsModifier Inputfile(File: " << filepath << ", ID: " << id << ")" << endl;
		Error = true;
	}
...
{
	return "#" + boost::regex_replace(string(bones), boost::regex("[^0-9]*([0-9]+).*"), string("\\1"));
}

bool hkbrigidbodyragdollcontrolsmodifier::IsBonesNull()
{
	if (bones.find("null", 0) != string::npos)
	{
		return true;
	}
	else
...
void hkbRigidBodyRagdollControlsModifierExport(string originalfile, string editedfile, string id)
{
	//stage 1 reading
	...
	{
		cout << "ERROR: Edit hkbRigidBodyRagdollControlsModifier Input Not Found (Original File: " << originalfile << ")" << endl;
		Error = true;
	}
	...
	{
		cout << "ERROR: Edit hkbRigidBodyRagdollControlsModifier Output Not Found (Edited File: " << editedfile << ")" << endl;
		Error = true;
	}
	...
	if (IsEdited)
	{
		...
		{
			cout << "ERROR: Edit hkbRigidBodyRagdollControlsModifier Output Not Found (New Edited File: " << editedfile << ")" << endl;
			Error = true;
		}
	}
	else
